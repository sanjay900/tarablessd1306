...
static const uint8_t *GetCharPtr(const struct SSD1306_FontDef *Font,
                                 char Character) {
  return &Font->FontData[(Character - Font->StartChar) *
                         ((Font->Width * (RoundUpFontHeight(Font) / 8)) + 1)];
}

void SSD1306_FontDrawChar(struct SSD1306_Device *DisplayHandle, char Character,
  ...
  if (Character >= DisplayHandle->Font->StartChar ||
      Character <= DisplayHandle->Font->EndChar) {
    /* The first byte in the glyph data is the width of the character in pixels,
     * skip over */
    GlyphData++;
    ...
    if (CharEndX < 0 || CharStartX >= DisplayHandle->Width || CharEndY < 0 ||
        CharStartY >= DisplayHandle->Height) {
      ClipDebug(x, y);
      return;
    }

    /* Do not attempt to draw past the end of the screen */
    CharEndX = (CharEndX >= DisplayHandle->Width) ? DisplayHandle->Width - 1
                                                  : CharEndX;
    CharEndY = (CharEndY >= DisplayHandle->Height) ? DisplayHandle->Height - 1
                                                   : CharEndY;
    ...
    for (x = CharStartX; x < CharEndX; x++) {
      for (y = CharStartY, i = 0; y < CharEndY && i < CharHeight; y++, i++) {
        ...
        if (GlyphData[YByte] & BIT(YBit)) {
          ...
          SSD1306_DrawPixel(DisplayHandle, x, y, SSD_COLOR_BLACK);
        }
      }
...
bool SSD1306_SetFont(struct SSD1306_Device *Display,
...
int SSD1306_FontGetCharWidth(struct SSD1306_Device *Display, char Character) {
  ...
  if (Character >= Display->Font->StartChar &&
      Character <= Display->Font->EndChar) {
    CharPtr = GetCharPtr(Display->Font, Character);

    Width =
        (Display->Font->Monospace == true) ? Display->Font->Width : *CharPtr;
...
int SSD1306_FontMeasureString(struct SSD1306_Device *Display,
                              const char *Text) {
  int Width = 0;
  int Len = 0;
  ...
  for (Len = strlen(Text); Len >= 0 && *Text != '\n'; Len--, Text++) {
...
void SSD1306_FontDrawString(struct SSD1306_Device *Display, int x, int y,
...
void SSD1306_FontDrawAnchoredString(struct SSD1306_Device *Display,
                                    TextAnchor Anchor, const char *Text,
                                    int Color) {
  int x = 0;
  int y = 0;
...
void SSD1306_FontGetAnchoredStringCoords(struct SSD1306_Device *Display,
                                         int *OutX, int *OutY,
                                         TextAnchor Anchor, const char *Text) {
  int StringWidth = 0;
  int StringHeight = 0;
